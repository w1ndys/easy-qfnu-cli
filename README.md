...
发布标签使用日期时间格式 `vYYYY.MM.DD.HHmm`，例如 `v2026.08.30.1430`。同一分钟如需重新发布，覆盖同名 Release 资产即可。发布通过仓库内的 `easy-qfnu-release` skill 和本机 `gh` 完成，不依赖 GitHub Actions 或仓库 Secret。
